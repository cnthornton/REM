...
class BankRule:
    ...
    def __init__(self, name, entry):
        ...
        try:
            table_entries = entry['DisplayTables']
        except KeyError:
            msg = 'missing required parameter "DisplayTables"'
            raise AttributeError(msg)

        self.account_tables = []
        self.association_tables = []
        for i, record_type in enumerate(table_entries):
            table_entry = table_entries[record_type]
            table = mod_elem.RecordTable(name, table_entry)
            self.account_tables.append(table)
            self.bindings.extend(table.bindings)

            table_entry['ActionButtons'] = {}
            assoc_table = mod_elem.RecordTable(name, table_entry)
            self.association_tables.append(assoc_table)
            self.bindings.extend(assoc_table.bindings)

            self.elements.append('-{NAME}_{ID}_Acct{TYPE}Panel-'.format(NAME=self.name, ID=self.id, TYPE=record_type))
            self.elements.append('-{NAME}_{ID}_Assoc{TYPE}Panel-'.format(NAME=self.name, ID=self.id, TYPE=record_type))
        ...
        for acct_id in accts:  # account entries
            ...
            try:
                acct_type = acct_entry['RecordType']
            except KeyError:
                msg = 'BankRule {RULE}: Account entry "{ACCT}" is missing the required parameter "RecordType"'\
                    .format(RULE=name, ACCT=acct_id)
                logger.error(msg)

                raise AttributeError(msg)

            try:
                table_entry = table_entries[acct_type]
            except KeyError:
                msg = 'BankRule {RULE}: the configured record type {TYPE} for account entry "{ACCT}" has no ' \
                      'corresponding display table configured'.format(RULE=name, TYPE=acct_type, ACCT=acct_id)
                logger.error(msg)

                raise AttributeError(msg)

            acct = BankAccount(acct_id, acct_entry, table_entry, parent=self.name)
    ...
    def fetch_table(self, identifier, by_key: bool = False, account: bool = True):
        """
        Fetch an account or association account display table.
        """
        if account:
            tables = self.account_tables
            tbl_type = 'Acct'
        else:
            tables = self.association_tables
            tbl_type = 'Assoc'

        if by_key:
            identifiers = [self.key_lookup('{TBL}{TYPE}Panel'.format(TBL=tbl_type, TYPE=i.record_type)) for i in tables]
        else:
            identifiers = [i.record_type for i in tables]

        if identifier in identifiers:
            index = identifiers.index(identifier)
            table = tables[index]
        else:
            raise KeyError('the display table with record type {TYPE} not found in the list of {NAME} tables'
                           .format(TYPE=identifier, NAME=self.name))
    ...
    def run_event(self, window, event, values):
        ...
        if current_acct:
            ...
            current_table = self.fetch_table(self.current_panel, by_key=True, account=True)
            acct_keys = current_table.bindings
            link_bttn = current_table.fetch_parameter('Link', filters=False)
        ...
        if current_assoc:
            ...
            current_assoc_table = self.fetch_table(self.current_assoc_panel, by_key=True, account=False)
            assoc_keys = current_assoc_table.bindings
        else:
            assoc_acct = None
            assoc_keys = []
        ...
        if event in (link_key, link_hkey):
            ...
            if ref_indices:
                ...
                ref_df = acct.collection.ref_df.copy()
                ...
                for panel in self.panels:
                    ...
                    ref_acct.update_references(ref_df.loc[ref_indices])
                ...
                if len(record_indices) > 1:
                    ...
                    if current_assoc:
                        ...
                        if not ref_ind.empty:
                            select_ind = assoc_table.record_index(ref_ind.tolist())
        ...
        # sub-panel.
            ...
            record_type = current_acct.record_type

            self.current_account = current_acct.name
            current_table = self.fetch_table(record_type, account=True)
            current_table.collection = current_acct.collection
            current_table.update_display(window)
            window[warn1_key].update(value='')

            panel_key = self.key_lookup('Acct{TYPE}Panel'.format(TYPE=record_type))
            if current_panel != panel_key:
                # Hide current panel and unhide new panel
        ...
        # display the relevant sub-panel.
            ...
            current_assoc_panel = self.current_assoc_panel
            ...
            record_type = assoc_acct.record_type

            self.current_association = assoc_acct.name
            current_assoc_table = self.fetch_table(record_type, account=False)
            current_assoc_table.collection = assoc_acct.collection
            current_assoc_table.update_display(window)
            window[warn2_key].update(value='')

            panel_key = self.key_lookup('Assoc{TYPE}Panel'.format(TYPE=record_type))
            if current_assoc_panel != panel_key:
                # Hide current panel and unhide new panel
                window[current_assoc_panel].update(visible=False)
                self.current_assoc_panel = panel_key
        ...
        # then load the relevant account record data
            ...
            if params:  # parameters were saved (selection not cancelled)
                ...
                current_table = self.fetch_table(self.current_panel, by_key=True)
                current_table.enable(window)
    ...
    def layout(self, size):
        ...
        tbl_w = int(frame_w / 2) - pad_v
        tbl_h = frame_h - header_h - warn_h - pad_h  # frame minus panel header row, warning, and vertical padding
        ...
        # Column 1 Panels
        panels = []
        for i, table in enumerate(self.account_tables):
            record_type = table.record_type
            panel_key = self.key_lookup('Acct{TYPE}Panel'.format(TYPE=record_type))
            tbl_layout = sg.Col([[table.layout(tooltip=self.title, size=(tbl_w, tbl_h), padding=(0, 0))]],
                                key=panel_key, pad=(pad_frame, pad_frame), justification='c',
                                vertical_alignment='t', background_color=bg_col, expand_x=True, visible=False)
            panels.append(tbl_layout)
        ...
        # Column 2 Panels
        panels = []
        for i, table in enumerate(self.association_tables):
            record_type = table.record_type
            panel_key = self.key_lookup('Assoc{TYPE}Panel'.format(TYPE=record_type))
            tbl_layout = sg.Col([[table.layout(tooltip=self.title, size=(tbl_w, tbl_h), padding=(0, 0))]],
                                key=panel_key, pad=(pad_frame, pad_frame), justification='c',
    ...
    def resize_elements(self, window, size):
        ...
        # Resize the table panels
        for table in self.account_tables:
            table.resize(window, (panel_w, panel_h))
    ...
    def link_records(self, acct_name, assoc_name: str = None):
        ...
        acct_table = self.fetch_table(self.current_panel, by_key=True, account=True)
        rows = acct_table.selected(real=True)

        if assoc_name:
            assoc_acct = self.fetch_account(assoc_name)
            assoc_table = self.fetch_table(self.current_assoc_panel, by_key=True, account=False)
            acct_rows = rows[0]
        ...
        record_id = acct_table.collection.row_ids(indices=acct_rows)[0]
        reference_id = assoc_table.collection.row_ids(indices=assoc_rows)[0]

        # Verify that the selected records do not already have references
        if acct.collection.has_reference(record_id) or assoc_acct.collection.has_reference(reference_id):
        ...
        if confirm:
            ...
            acct.collection.update_reference(record_id, reference_id, assoc_acct.record_type, approved=True, warning=user_note)
            assoc_acct.collection.update_reference(reference_id, record_id, acct.record_type, approved=True, warning=user_note)
    ...
    def link_records_new(self, acct_name, assoc_name: str = None):
        ...
        if acct.collection.has_reference(record_ids) or assoc_acct.collection.has_reference(reference_ids):
            msg = 'one or more of the selected records already have references'
            raise AssertionError(msg)
    ...
    def reconcile_statement(self, expand_search: bool = False, search_failed: bool = False):
        ...
        df = pd.merge(collection.data().drop(columns=list(acct.ref_map.values())), ref_df, how='left', on='RecordID')
        ...
        for index, row in df.iterrows():
            ...
            if nmatch == 1:  # found one exact match
                ...
                acct.collection.update_reference(record_id, ref_id, ref_type, approved=True)

                # Insert the reference into the associated account's reference dataframe
                assoc_acct = self.fetch_account(assoc_acct_name)
                assoc_acct.collection.update_reference(ref_id, record_id, acct.record_type, approved=True)
                ...
                # Insert the reference into the account records reference dataframe
                acct.collection.update_reference(record_id, ref_id, ref_type, approved=True, warning=warning)

                # Insert the reference into the associated account's reference dataframe
                assoc_acct = self.fetch_account(assoc_acct_name)
                assoc_acct.collection.update_reference(ref_id, record_id, acct.record_type, approved=True, warning=warning)
        ...
        if expand_search:
            ...
            for index, row in df.iterrows():
                ...
                # Insert the reference into the account records reference dataframe
                acct.collection.update_reference(record_id, ref_id, ref_type, approved=False, warning=warning)

                # Insert the reference into the associated account's reference dataframe
                assoc_acct = self.fetch_account(assoc_acct_name)
                assoc_acct.collection.update_reference(ref_id, record_id, acct.record_type, approved=False, warning=warning)
    ...
    def save_report(self, filename):
        ...
        with pd.ExcelWriter(filename) as writer:
            for acct_panel in self.panels:
                ...
                table = self.fetch_table(record_type)
                table.collection = acct.collection
...
class BankAccount:
    ...
    Bank transaction account entry.
        ...
        collection (ReferenceCollection): data collection for storing account data.
    ...
    def __init__(self, name, entry, table_entry, parent=None):
        """
        Arguments:
        ...
        try:
            ...
            msg = 'BankAccount {NAME}: missing required parameter "AssociationRule"'.format(NAME=self.name)
            logger.error(msg)

            raise AttributeError(msg)

        try:
            self.collection = mod_col.ReferenceCollection(self.name, table_entry)
        except AttributeError as e:
            msg = 'BankAccount {NAME}: failed to initialize account - {ERR}'.format(NAME=self.name, ERR=e)
            logger.error(msg)
        ...
        try:
            ...
            for transaction_acct in transactions:
                ...
                for assoc_column in list(assoc_params):
                    if assoc_column not in columns:
        ...
        if event in table_keys:
            ...
            if event in (open_key, return_key) and can_open:
                ...
                try:
                    ...
                    index = table.record_index(select_row_index)
                    ...
                    if can_open:
                        ...
                        if can_edit:  # only update references if table is editable
                            ...
                            try:
                                ...
                                table.update_entry(index, record_values)
            ...
            indices = table.record_index(select_row_indices)
            ...
            if 'Approved' in colmap:
                table.update_field(colmap['Approved'], True, indices=indices)
    ...
    def update_references(self, ref_df):
        ...
        df = self.collection.ref_df.copy()
    ...
    def approve(self, record_ids):
        ...
        ref_indices = collection.record_index(record_ids)
        collection.set_state('approved', True, indices=ref_indices, reference=True)
    ...
    def reset_records(self, identifiers, index: bool = True):
        ...
        if index:
            ...
            indices = collection.record_index(identifiers)

        if 'Void' in colmap:
            table.update_field(colmap['Void'], False, indices=indices)

        if 'Approved' in colmap:
            collection.update_field(colmap['Approved'], False, indices=indices)
    ...
    def search_void(self, df):
        """
        Find void transactions and set the correct transaction type.
        """
        pd.set_option('display.max_columns', None)
        ...
        if bc_entry:
            ...
            for matching_inds in matches:
                ...
                collection.update_reference(record_id, ref_id, record_type, warning=warning)
                collection.update_reference(ref_id, record_id, record_type, warning=warning)
        ...
        void_inds = collection.record_index(void_records)
        if len(void_inds) > 0:
            print('BankAccount {}: setting void column {} to true for indices:'.format(self.name, failed_col))
            print(void_inds)
            collection.update_field(failed_col, True, indices=void_inds)
    ...
    def filter_void(self, df: pd.DataFrame = None):
        ...
        df = df if df is not None else collection.data()
    ...
    def load_data(self, params, records: list = None):
        ...
        record_ids = df[collection.id_column].tolist()
        ...
        # Update the account data with the data imported from the database
        collection.append(df, ref_df)
    ...
    def load_records(self, parameters, records: list = None):
        ...
        imported_ids = df[self.collection.id_column].tolist()
        if records:
