...
class TabItem:
    def __init__(self, rule_name, name, tdict):
        ...
        ncol = len(header)
        self.df = pd.DataFrame(mod_dm.create_empty_table(nrow=20, ncol=ncol),
                               columns=header)  # initialize with empty table
    ...
    def reset_dynamic_attributes(self):
        ...
        """
        header = [mod_dm.colname_from_query(i) for i in self.db_columns]
        ncol = len(header)
        data = mod_dm.create_empty_table(nrow=20, ncol=ncol)
    ...
    def update_table(self, window):
        ...
        df = mod_dm.sort_table(self.df, self.db_key)
    ...
    def update_summary(self, window):
        ...
        for rule_name in summ_rules:
            ...
            if 'Subset' in summ_rule:
                try:
                    subset_df = mod_dm.subset_dataframe(df, summ_rule['Subset'])
    ...
    def layout(self, win_size: tuple = None):
        ...
        pad_v = pad_h = mod_const.HORZ_PAD

        font_l = mod_const.LARGE_FONT
        font_m = mod_const.MID_FONT
        ...
        data = mod_dm.create_empty_table(nrow=20, ncol=len(header))
    ...
    def search_for_errors(self):
        ...
        results = mod_dm.evaluate_rule_set(df, error_rules)  # returns list of booleans
    ...
    def run_audit(self, *args, **kwargs):
        ...
        for action in self.actions:
            ...
            try:
                ...
                mod_win2.popup_error('Error: audit scan failed - {}'.format(e))
    ...
    def scan_for_missing(self, *args, **kwargs):
        ...
        df = mod_dm.sort_table(self.df, pkey)
        ...
        for audit_param in audit_params:
            if audit_param.type.lower() == 'date':
                ...
                date_col_full = mod_dm.get_query_from_header(date_col, self.db_columns)
        ...
        if missing_transactions:
            pkey_fmt = mod_dm.get_query_from_header(pkey, self.db_columns)
        ...
        if not missing_df.empty:
            ...
            if not import_df.empty:
                df = mod_dm.append_to_table(self.df, import_df)
...
def importer_layout(win_size: tuple = None):
    ...
    record_types = configuration.records.print_entries(by_title=True)
